...
export function verifyMetaSignature(rawBody, sigHeader) {
  ...
  if (!appSecret) {
    console.warn(
      "⚠️  META_APP_SECRET not set — skipping Meta signature verification. " +
      "This MUST be set in production (Vercel env vars)."
    );
    return true; // allow through in local dev without the secret
  }
  if (!sigHeader) {
...
export function verifyOwnershipHmac(phoneNumberId, userId, storedHmac) {
  ...
  if (!expected) return true; // WEBHOOK_SECRET not configured — skip
