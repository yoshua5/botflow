...
export async function getConfig(explicitId, botId) {
  ...
  if (USE_SUPABASE) {
    ...
    // Apply env vars ONLY as fallback (never override per-bot DB values).
    // Exception: anthropicKey can come from env if not set per-bot (platform key).
    // WA_ACCESS_TOKEN / WA_PHONE_NUMBER_ID must NEVER override DB — each bot
    // has its own token. Keeping them here would break multi-tenancy.
    const env = envOverrides();
    const merged = { ...stored };
    for (const [k, v] of Object.entries(env)) {
      // WhatsApp credentials must come from DB only
      if (k === "accessToken" || k === "phoneNumberId" || k === "waBusinessId") continue;
      // Everything else (anthropicKey, verifyToken, openaiKey) is a fallback
...
export async function trackMessage(from, message, botName, explicitId, botId, contactName) {
  const uid   = await getUserIdAndEnsure(explicitId);
  ...
  if (USE_SUPABASE) {
    ...
    // Update contact_name in conversations table if we have a name
    if (contactName) {
