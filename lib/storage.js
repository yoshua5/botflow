...
function envOverrides() {
  const o = {};
  if (process.env.ANTHROPIC_KEY)      o.anthropicKey    = process.env.ANTHROPIC_KEY;
...
export async function setConfig(config, explicitId, botId) {
  ...
  if (USE_SUPABASE) {
    ...
    if (!resolvedBotId) return; // No bot to save to
  ...
  fs.writeFileSync(
    path.join(process.cwd(), `botflow-config_${uid}.json`),
...
export async function getBots(explicitId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), `botflow-bots_${uid}.json`), "utf-8"));
  } catch { return []; }
}
...
export async function setBots(bots, explicitId) {
  ...
  const fs   = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), `botflow-bots_${uid}.json`), JSON.stringify(bots, null, 2));
}
...
export async function getKBIndex(explicitId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    const p    = path.join(process.cwd(), "knowledge-base", `index_${uid}.json`);
...
export async function getKBText(key, explicitId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    const p    = path.join(process.cwd(), "knowledge-base", `${uid}_${key}`);
...
export async function setKBText(key, text, explicitId) {
  const uid = await getUserIdAndEnsure(explicitId);
  const safe = text.length > 200_000 ? text.slice(0, 200_000) + "\n[...truncado]" : text;
...
export async function deleteRawFile(id, explicitId) {
  ...
  try {
    ...
    const path = (await import("path")).default;
    const uid  = getUid(explicitId);
    const p    = path.join(process.cwd(), "knowledge-base", `${uid}_${id}`);
    if (fs.existsSync(p)) fs.unlinkSync(p);
  } catch {}
}
...
export async function getConversation(from, explicitId, botId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    const p    = path.join(process.cwd(), "knowledge-base", `conv_${uid}_${from}.json`);
...
export async function getAnalytics(explicitId, botId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), `botflow-analytics_${uid}.json`), "utf-8"));
...
export async function setAnalytics(data, explicitId, botId) {
  ...
  const fs   = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), `botflow-analytics_${uid}.json`), JSON.stringify(data, null, 2));
}
...
export async function getSubscription(explicitId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), `subscription_${uid}.json`), "utf-8"));
  } catch { return null; }
}
...
export async function setSubscription(data, explicitId) {
  ...
  const fs   = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), `subscription_${uid}.json`), JSON.stringify(data, null, 2));
}
...
export async function getUserIdByStripeCustomer(customerId) {
  ...
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), `stripe-customer_${customerId}.json`), "utf-8")).userId;
  } catch { return null; }
}

export async function setStripeCustomerMapping(customerId, userId) {
  // In Supabase this is already stored in subscriptions.stripe_customer_id
  if (USE_SUPABASE) return;
  const IS_VERCEL = !!process.env.KV_REST_API_URL;
  if (IS_VERCEL) { const store = await kv(); await store.set(`stripe-customer:${customerId}`, userId); return; }
  const fs   = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), `stripe-customer_${customerId}.json`), JSON.stringify({ userId }));
}

// ═══════════════════════════════════════════════════════════
// LANDING / GLOBAL CONFIG (admin only)
// ═══════════════════════════════════════════════════════════

export async function getGlobalLanding() {
  const IS_VERCEL = !!process.env.KV_REST_API_URL;
  if (IS_VERCEL) { const store = await kv(); return (await store.get("global:landing")) || {}; }
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), "global_landing.json"), "utf-8"));
  } catch { return {}; }
}

export async function setGlobalLanding(data) {
  const IS_VERCEL = !!process.env.KV_REST_API_URL;
  if (IS_VERCEL) { const store = await kv(); await store.set("global:landing", data); return; }
  const fs   = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), "global_landing.json"), JSON.stringify(data, null, 2));
}
