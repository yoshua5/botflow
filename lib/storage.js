...
async function localReadBots(uid) {
  try {
    ...
    const p = path.join(process.cwd(), `botflow-bots_${uid}.json`);
...
async function localReadConfig(uid) {
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    const p = path.join(process.cwd(), `botflow-config_${uid}.json`);
    if (!fs.existsSync(p)) return {};
    return JSON.parse(fs.readFileSync(p, "utf-8"));
  } catch { return {}; }
}
...
async function localWriteConfig(uid, config) {
  try {
    const fs   = (await import("fs")).default;
    const path = (await import("path")).default;
    fs.writeFileSync(path.join(process.cwd(), `botflow-config_${uid}.json`), JSON.stringify(config, null, 2));
  } catch (e) {
...
export async function getGlobalLanding() {
  ...
  try {
    const fs = (await import("fs")).default;
    const path = (await import("path")).default;
    return JSON.parse(fs.readFileSync(path.join(process.cwd(), "botflow-landing.json"), "utf-8"));
  } catch { return {}; }
}

export async function setGlobalLanding(data) {
  const IS_VERCEL = !!process.env.KV_REST_API_URL;
  if (IS_VERCEL) { const store = await kv(); await store.set("botflow-landing", data); return; }
  const fs = (await import("fs")).default;
  const path = (await import("path")).default;
  fs.writeFileSync(path.join(process.cwd(), "botflow-landing.json"), JSON.stringify(data, null, 2));
}
