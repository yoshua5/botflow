...
export default function UserDetailPage() {
  ...
  if (loading) return <div style={{ color:"#475569",padding:40,textAlign:"center" }}>Cargando usuario...</div>;
  if (!user)   return (
    <div style={{ textAlign:"center",padding:60 }}>
      <div style={{ fontSize:40,marginBottom:12 }}>❓</div>
      <div style={{ color:"#475569" }}>Usuario no encontrado</div>
      <Link href="/dashboard/super-admin/users" style={{ color:"#6366F1",marginTop:8,display:"block" }}>← Volver</Link>
    </div>
  );
  ...
  return (
    <div>
      ...
      {/* Back */}
      <Link href="/dashboard/super-admin/users" style={{ color:"#475569",fontSize:13,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:6,marginBottom:20 }}>
        ← Todos los usuarios
      </Link>
      ...
      <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24,marginBottom:20 }}>
        <div style={{ display:"flex",alignItems:"center",gap:20,flexWrap:"wrap" }}>
          <div style={{ width:56,height:56,borderRadius:"50%",background:"linear-gradient(135deg,#6366F1,#8B5CF6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:800,color:"#fff",flexShrink:0 }}>
            {(user.email||"?")[0].toUpperCase()}
          </div>
          <div style={{ flex:1 }}>
            <h2 style={{ margin:0,fontSize:20,fontWeight:800,color:"#fff" }}>{user.name || user.email.split("@")[0]}</h2>
            ...
            <div style={{ display:"flex",gap:8,marginTop:8,flexWrap:"wrap" }}>
              ...
              <span style={{ fontSize:11,color:"#475569" }}>Registrado {user.created_at ? new Date(user.created_at).toLocaleDateString("es") : "—"}</span>
              <span style={{ fontSize:11,color:"#475569" }}>{bots.length} bots</span>
            </div>
          </div>
          {/* Quick actions */}
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
            <select onChange={e=>{ if(e.target.value) doUserAction("change_plan",{ plan:e.target.value }); e.target.value=""; }} defaultValue=""
              style={{ padding:"8px 12px",background:"#0A0A0F",border:"1px solid #2D2D44",borderRadius:8,color:"#94A3B8",fontSize:12,cursor:"pointer" }}>
            ...
            <button onClick={()=>doUserAction(user.subStatus==="suspended"?"reactivate":"suspend")} disabled={actionLoading}
              style={{ padding:"8px 14px",background:"#1A1A2E",border:"1px solid #2D2D44",borderRadius:8,color:"#F59E0B",fontSize:12,fontWeight:600,cursor:"pointer" }}>
              {user.subStatus==="suspended"?"✅ Activar":"⏸️ Suspender"}
            </button>
            <button onClick={()=>{ if(confirm("¿Eliminar usuario?")) doUserAction("delete"); }}
              style={{ padding:"8px 14px",background:"#1A1A2E",border:"1px solid #EF444422",borderRadius:8,color:"#EF4444",fontSize:12,fontWeight:600,cursor:"pointer" }}>
              🗑️ Eliminar
            </button>
          </div>
        </div>
      ...
      {tab === "overview" && (
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
          {[
            ...
            <div key={k} style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:10,padding:"16px 20px" }}>
              <div style={{ fontSize:11,color:"#475569",fontWeight:700,textTransform:"uppercase",marginBottom:4 }}>{k}</div>
      ...
      {tab === "bots" && (
        <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,overflow:"hidden" }}>
          {bots.length === 0 && <div style={{ padding:40,textAlign:"center",color:"#334155" }}>Este usuario no tiene bots</div>}
          {bots.map(b => (
            <div key={b.id} style={{ display:"flex",alignItems:"center",gap:16,padding:"14px 20px",borderBottom:"1px solid #0F0F1A" }}>
              <div style={{ width:36,height:36,borderRadius:8,background:"#1A1A2E",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18 }}>🤖</div>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:600,color:"#E2E8F0" }}>{b.name}</div>
                <div style={{ fontSize:11,color:"#475569" }}>@{b.handle} · {b.status}</div>
              </div>
      ...
      {tab === "notes" && (
        <div>
          {/* Add note */}
          <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:20,marginBottom:16 }}>
            <div style={{ fontSize:13,fontWeight:600,color:"#94A3B8",marginBottom:10 }}>Nueva Nota Interna</div>
            <textarea value={newNote} onChange={e=>setNewNote(e.target.value)} rows={3}
              placeholder="Escribe una nota sobre este usuario..."
              style={{ width:"100%",background:"#0A0A0F",border:"1px solid #2D2D44",borderRadius:8,padding:12,color:"#E2E8F0",fontSize:13,resize:"vertical",outline:"none",boxSizing:"border-box" }} />
            <button onClick={addNote} style={{ marginTop:8,padding:"8px 18px",background:"#6366F1",border:"none",borderRadius:8,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer" }}>
              Guardar Nota
            </button>
          </div>
          {/* Notes list */}
          {notes.length === 0 && <div style={{ color:"#334155",fontSize:13,padding:20 }}>Sin notas</div>}
          {notes.map(n => (
            <div key={n.id} style={{ background:"#111118",border:`1px solid ${n.is_pinned?"#F59E0B44":"#1E1E2E"}`,borderRadius:10,padding:16,marginBottom:10,position:"relative" }}>
              {n.is_pinned && <div style={{ position:"absolute",top:10,right:10,fontSize:12 }}>📌</div>}
              <div style={{ fontSize:13,color:"#CBD5E1",lineHeight:1.6 }}>{n.content}</div>
              <div style={{ display:"flex",gap:8,marginTop:10,alignItems:"center" }}>
                <span style={{ fontSize:11,color:"#334155" }}>{new Date(n.created_at).toLocaleString("es")}</span>
