  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:22,fontWeight:800,color:"#0F172A",margin:0 }}>Landing Page CMS</h1>
          <div style={{ fontSize:13,color:"#64748B",marginTop:4 }}>Edita el contenido del sitio en tiempo real</div>
        </div>
        <div style={{ display:"flex",gap:10 }}>
          <a href="/" target="_blank" rel="noreferrer" style={{ padding:"9px 16px",background:"#F8FAFF",border:"1px solid #2D2D44",borderRadius:8,color:"#64748B",fontSize:13,textDecoration:"none",fontWeight:600 }}>
            👁️ Ver Landing
          </a>
          <button onClick={save} disabled={saving} style={{ padding:"9px 18px",background:"#2563EB",border:"none",borderRadius:8,color:"#0F172A",fontSize:13,fontWeight:600,cursor:"pointer" }}>
      ...
      <div style={{ background:"#FFFFFF",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
        ...
        {tab === "stats" && (
          <div>
            <div style={{ fontSize:13,color:"#64748B",marginBottom:16 }}>Estadísticas mostradas en la barra de prueba social</div>
            {(config.stats||[]).map((s,i)=>(
              <div key={i} style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10,padding:12,background:"#F8FAFF",borderRadius:8 }}>
                <div>
                  ...
                  <input value={s.label} onChange={e=>setConfig(c=>{ const stats=[...c.stats]; stats[i]={...stats[i],label:e.target.value}; return {...c,stats}; })}
                    style={{ width:"100%",background:"#FFFFFF",border:"1px solid #2D2D44",borderRadius:6,padding:"6px 10px",color:"#0F172A",fontSize:13,outline:"none",boxSizing:"border-box" }} />
                </div>
                <div>
                  <label style={{ fontSize:11,color:"#64748B",fontWeight:700,display:"block",marginBottom:4 }}>Valor</label>
        ...
        {tab === "pricing" && (
          <div>
            {["starter","pro","enterprise"].map(plan=>(
              <div key={plan} style={{ marginBottom:20,padding:16,background:"#F8FAFF",borderRadius:10 }}>
                <div style={{ fontWeight:700,color:"#0F172A",marginBottom:10,textTransform:"capitalize" }}>{plan}</div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
                  <div>
                    <label style={{ fontSize:11,color:"#64748B",fontWeight:700,display:"block",marginBottom:4 }}>Precio (USD/mes)</label>
                    <input value={config.pricing?.[plan]?.price||""} onChange={e=>setNested(`pricing.${plan}.price`,e.target.value)}
                      style={{ width:"100%",background:"#FFFFFF",border:"1px solid #2D2D44",borderRadius:6,padding:"6px 10px",color:"#0F172A",fontSize:13,outline:"none",boxSizing:"border-box" }} />
                  </div>
                  <div>
                    <label style={{ fontSize:11,color:"#64748B",fontWeight:700,display:"block",marginBottom:4 }}>Descripción</label>
