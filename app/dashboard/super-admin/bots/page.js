  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24 }}>
        <div>
          <h1 style={{ fontSize:22,fontWeight:800,color:"#fff",margin:0 }}>Gestión de Bots</h1>
          <div style={{ fontSize:13,color:"#475569",marginTop:4 }}>{bots.length} bots en la plataforma</div>
        </div>
      </div>
      <div style={{ marginBottom:16 }}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar bots..."
          style={{ padding:"9px 14px",background:"#111118",border:"1px solid #2D2D44",borderRadius:8,color:"#E2E8F0",fontSize:13,outline:"none",width:300 }} />
      </div>
          ...
          <thead>
            <tr style={{ borderBottom:"1px solid #1E1E2E" }}>
              {["Bot","Usuario","Estado","WhatsApp","Creado","Acciones"].map(h=>(
                <th key={h} style={{ padding:"12px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#475569",textTransform:"uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading && <tr><td colSpan={6} style={{ padding:40,textAlign:"center",color:"#334155" }}>Cargando...</td></tr>}
            {!loading && bots.length === 0 && <tr><td colSpan={6} style={{ padding:40,textAlign:"center",color:"#334155" }}>No hay bots</td></tr>}
            {bots.map(b=>(
              <tr key={b.id} style={{ borderBottom:"1px solid #0F0F1A" }}
                ...
                <td style={{ padding:"12px 16px" }}>
                  ...
                  <div style={{ fontSize:11,color:"#475569" }}>@{b.handle}</div>
                </td>
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ fontSize:12,color:"#94A3B8" }}>{b.userEmail}</div>
                </td>
                <td style={{ padding:"12px 16px" }}>
                ...
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ display:"flex",gap:6 }}>
                    <button onClick={()=>doAction(b.id, b.status==="active"?"disable":"enable")}
                      style={{ padding:"5px 10px",background:"#1A1A2E",border:"1px solid #2D2D44",borderRadius:6,color:b.status==="active"?"#F59E0B":"#22C55E",fontSize:11,fontWeight:600,cursor:"pointer" }}>
                      {b.status==="active"?"⏸️":"▶️"}
                    </button>
                    <button onClick={()=>{ if(confirm("¿Eliminar bot?")) doAction(b.id,"delete"); }}
                      style={{ padding:"5px 10px",background:"#1A1A2E",border:"1px solid #EF444422",borderRadius:6,color:"#EF4444",fontSize:11,cursor:"pointer" }}>
                      🗑️
                    </button>
                  </div>
                </td>
