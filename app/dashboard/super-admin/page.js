...
function KPI({ label, value, sub, icon, color, href }) {
  const card = (
    <div style={{
      background:"#111118", border:"1px solid #1E1E2E", borderRadius:12, padding:"20px 24px",
      cursor: href ? "pointer" : "default",
      transition:"border-color .2s",
    }}
    onMouseEnter={e => href && (e.currentTarget.style.borderColor="#6366F1")}
    onMouseLeave={e => href && (e.currentTarget.style.borderColor="#1E1E2E")}
    >
      <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:12 }}>
        <div style={{ fontSize:22 }}>{icon}</div>
        <div style={{ fontSize:11,fontWeight:600,color:color||"#6366F1",background:`${color||"#6366F1"}22`,padding:"2px 8px",borderRadius:20 }}>
          {sub}
        </div>
      </div>
...
function ActivityItem({ type, label, time }) {
  ...
  const colors = { user:"#22C55E", bot:"#6366F1", payment:"#10B981", upgrade:"#F59E0B", failed:"#EF4444", broadcast:"#8B5CF6" };
  return (
    <div style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #1A1A2E" }}>
      <div style={{ width:32,height:32,borderRadius:8,background:`${colors[type]||"#475569"}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0 }}>
        {icons[type]||"•"}
      </div>
      <div style={{ flex:1,minWidth:0 }}>
        <div style={{ fontSize:13,color:"#CBD5E1",fontWeight:500 }}>{label}</div>
        <div style={{ fontSize:11,color:"#334155" }}>{time}</div>
      </div>
    </div>
  );
}
...
function MiniChart({ data, color }) {
  if (!data || data.length === 0) return <div style={{ color:"#334155",fontSize:12 }}>Sin datos</div>;
  const max = Math.max(...data.map(d=>d.count), 1);
  return (
    <div style={{ display:"flex",alignItems:"flex-end",gap:2,height:40 }}>
      {data.slice(-20).map((d,i) => (
        <div key={i} title={`${d.date}: ${d.count}`}
          style={{ flex:1,background:color||"#6366F1",borderRadius:"2px 2px 0 0",opacity:.7+.3*(d.count/max),
  ...
  if (loading) return (
    <div style={{ display:"flex",alignItems:"center",justifyContent:"center",height:"60vh" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <div style={{ color:"#475569" }}>Cargando métricas...</div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28 }}>
        <div>
          <h1 style={{ fontSize:24,fontWeight:800,color:"#fff",margin:0 }}>Control Center</h1>
          <div style={{ fontSize:13,color:"#475569",marginTop:4 }}>AgentFlow Super Admin · Actualizado {timeStr}</div>
        </div>
        <button onClick={fetchMetrics} style={{ background:"#1A1A2E",border:"1px solid #2D2D44",borderRadius:8,padding:"8px 16px",color:"#818CF8",fontSize:13,cursor:"pointer" }}>
          ↻ Actualizar
        </button>
      </div>

      {/* KPI Grid — row 1: users */}
      <div style={{ fontSize:11,fontWeight:700,color:"#475569",letterSpacing:1,marginBottom:10,textTransform:"uppercase" }}>Usuarios</div>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:24 }}>
        <KPI icon="👥" label="Total Usuarios"   value={m.totalUsers}    sub="total"   color="#6366F1" href="/dashboard/super-admin/users" />
      ...
      <div style={{ fontSize:11,fontWeight:700,color:"#475569",letterSpacing:1,marginBottom:10,textTransform:"uppercase" }}>Plataforma</div>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:12,marginBottom:24 }}>
        ...
        <KPI icon="💬" label="Conversaciones"   value={m.conversationsProcessed} sub="total" color="#6366F1" />
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 380px",gap:20,marginBottom:24 }}>
        ...
        <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16 }}>
            <div style={{ fontWeight:700,fontSize:15,color:"#fff" }}>Crecimiento de Usuarios (30 días)</div>
            <div style={{ fontSize:12,color:"#475569" }}>+{m.newUsersMonth} este mes</div>
          </div>
          <MiniChart data={m.growth || []} color="#6366F1" />
          {(m.growth||[]).length === 0 && (
            <div style={{ height:40,display:"flex",alignItems:"center",justifyContent:"center",color:"#334155",fontSize:12 }}>
        ...
        {/* Plan distribution */}
        <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
          <div style={{ fontWeight:700,fontSize:15,color:"#fff",marginBottom:16 }}>Distribución de Planes</div>
          {Object.entries(m.planDistribution || { free:0, starter:0, pro:0, enterprise:0 }).map(([plan,count]) => {
            const colors2 = { free:"#475569", starter:"#2563EB", pro:"#7C3AED", enterprise:"#0F172A" };
            ...
            return (
              <div key={plan} style={{ marginBottom:12 }}>
                <div style={{ display:"flex",justifyContent:"space-between",marginBottom:4 }}>
                  <span style={{ fontSize:12,color:"#94A3B8",fontWeight:600 }}>{labels2[plan]||plan}</span>
                  <span style={{ fontSize:12,color:"#64748B" }}>{count} ({pct}%)</span>
                </div>
                <div style={{ height:6,background:"#1A1A2E",borderRadius:3 }}>
                  <div style={{ height:"100%",width:`${pct}%`,background:colors2[plan]||"#6366F1",borderRadius:3,transition:"width .5s" }} />
                </div>
              </div>
            );
          })}
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20 }}>
        <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
          <div style={{ fontWeight:700,fontSize:15,color:"#fff",marginBottom:4 }}>Bots Recientes</div>
          <div style={{ fontSize:12,color:"#475569",marginBottom:16 }}>Últimos bots creados</div>
          {(m.recentBots || []).length === 0 && <div style={{ color:"#334155",fontSize:13 }}>Sin actividad reciente</div>}
          {(m.recentBots || []).map(b => (
            <ActivityItem key={b.id} type="bot" label={`Bot "${b.name}" creado`} time={new Date(b.created_at).toLocaleDateString("es")} />
          ))}
        </div>
        <div style={{ background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
          <div style={{ fontWeight:700,fontSize:15,color:"#fff",marginBottom:4 }}>Suscripciones Recientes</div>
          <div style={{ fontSize:12,color:"#475569",marginBottom:16 }}>Últimos cambios de plan</div>
          {(m.recentSubscriptions || []).length === 0 && <div style={{ color:"#334155",fontSize:13 }}>Sin actividad reciente</div>}
      ...
      <div style={{ marginTop:24,background:"#111118",border:"1px solid #1E1E2E",borderRadius:12,padding:24 }}>
        <div style={{ fontWeight:700,fontSize:15,color:"#fff",marginBottom:16 }}>Acciones Rápidas</div>
        <div style={{ display:"flex",flexWrap:"wrap",gap:10 }}>
          {[
            ...
            <Link key={a.href} href={a.href} style={{
              display:"flex",alignItems:"center",gap:8,padding:"10px 16px",background:"#0A0A0F",
              border:"1px solid #2D2D44",borderRadius:8,textDecoration:"none",color:"#94A3B8",fontSize:13,fontWeight:500,
            }}>
              {a.icon} {a.label}
            </Link>
          ))}
