...
export async function GET(request) {
  try {
    ...
    // Return default fields if none configured yet
    if (!data || data.length === 0) {
      return NextResponse.json({ fields: getDefaultFields() });
    }
    return NextResponse.json({ fields: data });
  } catch (err) {
    console.error("GET /api/citas/fields error:", err);
  ...
  try {
    ...
    if (fields.length > 0) {
      const rows = fields.map((f, i) => ({
        ...
        field_label: f.field_label || f.field_key || `Campo ${i + 1}`,
        question:    f.question   || f.field_label || `¿Cuál es tu ${f.field_label}?`,
        field_order: i,
    ...
    console.error("POST /api/citas/fields error:", err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
